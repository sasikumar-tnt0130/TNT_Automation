...
class MPLegacyReservationSetup:
    ...
    in from secrets.ini's global `[payment]` section (merged by load_config).
    ...
    def assert_confirmation_shows_move_in_date(self, move_in_date: date | None = None) -> None:
        """Confirmation page text includes the selected move-in date."""
        check_date = move_in_date or self.move_in_date
        if not check_date:
            raise AssertionError("No move_in_date to assert on the confirmation page")
        body = self.legacy_page.page.locator("body")
        patterns = [
